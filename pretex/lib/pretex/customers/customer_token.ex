defmodule Pretex.Customers.CustomerToken do
  use Ecto.Schema
  import Ecto.Query
  alias Pretex.Customers.CustomerToken
  ...
  def build_session_token(customer) do
    ...
    dt = customer.authenticated_at || DateTime.utc_now(:second)
  ...
  The query returns the customer found by the token, if any, along with the token's creation time.
  ...
  def verify_session_token_query(token) do
    query =
      from token in by_token_and_context_query(token, "session"),
        ...
        select: {%{customer | authenticated_at: token.authenticated_at}, token.inserted_at}
  ...
  the token in the application to gain access. Furthermore, if the customer changes
  ...
  If found, the query returns a tuple of the form `{customer, token}`.
      ...
      {:ok, decoded_token} ->
        ...
        query =
          from token in by_token_and_context_query(hashed_token, "login"),
            ...
            where: token.sent_to == customer.email,
            select: {customer, token}
  ...
  The query returns the customer_token found by the token, if any.
